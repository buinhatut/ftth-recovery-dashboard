...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    loadTn1Baseline(token);
  }, [user, selectedReportDate, selectedVTKV]);
  ...
  async function loadTn1Baseline(token: string) {
    try {
      const samePeriodDate = getPrevMonthSameDay(selectedReportDate);

      const res = await fetch(
        `/api/proxy?action=getTn1Baseline&token=${encodeURIComponent(
          token
        )}&report_date=${encodeURIComponent(
          samePeriodDate
        )}&vtkv=${encodeURIComponent(selectedVTKV || "ALL")}`,
        { cache: "no-store" }
      );
  ...
  const yesterday = getPreviousDate(selectedReportDate);
  const samePeriodDate = getPrevMonthSameDay(selectedReportDate);

  const monthToDateRows = useMemo(() => {
    const reportMonth = selectedReportDate.slice(0, 7);
    ...
    return viewRows.filter((r) => {
      ...
      return d.slice(0, 7) === reportMonth && d <= selectedReportDate;
    });
  }, [viewRows, selectedReportDate]);
  ...
  const monthStats = useMemo(() => buildStats(monthToDateRows), [monthToDateRows]);
  ...
  const samePeriodDelta = monthStats.total - tn1SamePeriodTotal;
  const samePeriodPct = tn1SamePeriodTotal
    ? Math.round((samePeriodDelta / tn1SamePeriodTotal) * 1000) / 10
    : monthStats.total > 0
    ? 100
    : 0;
  ...
  const trendData = useMemo(() => {
    ...
    return data;
  }, [monthToDateRows, activeMonth]);

  const byVTKV = useMemo(() => groupBy(monthToDateRows, "vtkv"), [monthToDateRows]);
  const byCNKD = useMemo(() => groupBy(monthToDateRows, "cnkd"), [monthToDateRows]);
  ...
  const reasonL1Chart = useMemo(() => {
    return topCount(monthToDateRows, "latest_reason_l1_name", 10);
  }, [monthToDateRows]);

  const reasonL2Chart = useMemo(() => {
    return topCount(monthToDateRows, "latest_reason_l2_name", 10);
  }, [monthToDateRows]);
  ...
  function goList(filter: string) {
    ...
    if (selectedReportDate) params.set("report_date", selectedReportDate);
              ...
              <div ref={exportRef} className="bg-white p-4">
                <div className="mb-5">
                  ...
                  <p className="text-sm text-slate-500">
                    Ngày báo cáo: {formatDateVi(selectedReportDate)} | Tháng báo cáo: {activeMonth} | Thời điểm xuất:{" "}
                    {new Date().toLocaleString("vi-VN")}
                  </p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-9 gap-5 mb-6">
                  <MainCard
                  ...
                  <MainCard
                    title="So cùng kỳ Tn-1"
                    value={`${samePeriodDelta >= 0 ? "+" : ""}${num(samePeriodDelta)}`}
                    subtitle={`${samePeriodDelta >= 0 ? "+" : ""}${samePeriodPct}% so với cùng kỳ`}
                    color={samePeriodDelta > 0 ? COLORS.red : samePeriodDelta < 0 ? COLORS.green : COLORS.purple}
                    extra={[
                ...
                <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 mb-6">
                  ...
                  <section className="bg-white rounded-2xl shadow p-6">
                    ...
                    <Summary label="Cùng kỳ Tn-1" value={tn1SamePeriodTotal} />
                    <Summary
            ...
            <>
              <div className="bg-white rounded-2xl shadow p-4 mb-4 grid grid-cols-2 gap-3">
                <input
                  ...
                  value={selectedReportDate}
                  onChange={(e) => setSelectedReportDate(e.target.value)}
                  className="col-span-2 border border-slate-300 rounded-xl px-3 py-3 bg-white"
                />
              ...
              <div className="mb-4">
                ...
                <p className="text-sm text-slate-500">
                  Ngày báo cáo: {formatDateVi(selectedReportDate)} | Tháng: {activeMonth}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <MobileKpiCard
                  ...
                  value={`${samePeriodDelta >= 0 ? "+" : ""}${num(samePeriodDelta)}`}
                  subtitle={`${samePeriodDelta >= 0 ? "+" : ""}${samePeriodPct}% so với cùng kỳ`}
                  color={samePeriodDelta > 0 ? "text-red-600" : samePeriodDelta < 0 ? "text-green-600" : "text-slate-700"}
                />
...
function getPreviousDate(dateKey: string) {
  const d = parseLocalDate(dateKey) || new Date();
  d.setDate(d.getDate() - 1);
  return toDateKey(d);
}

function getPrevMonthSameDay(dateKey: string) {
  const d = parseLocalDate(dateKey) || new Date();

  const targetDay = d.getDate();
  const prev = new Date(d.getFullYear(), d.getMonth() - 1, 1);
  const lastDayPrevMonth = new Date(prev.getFullYear(), prev.getMonth() + 1, 0).getDate();

  prev.setDate(Math.min(targetDay, lastDayPrevMonth));
