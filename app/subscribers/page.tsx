...
export default function SubscribersPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (filter === "failed") {
        ok = result === "FAILED" || result === "NOT_RECOVERED";
      }
  ...
  function getFilterTitle() {
    if (filter === "contacted") {
      return "Danh sách thuê bao đã tiếp xúc";
    }

    if (filter === "not_contacted") {
      return "Danh sách thuê bao chưa tiếp xúc";
    }

    if (filter === "recovered") {
      return "Danh sách thuê bao đã khôi phục";
    }

    if (filter === "failed") {
      return "Danh sách thuê bao không khôi phục";
    }

    if (filter === "pending") {
      return "Danh sách thuê bao đang xử lý";
    }

    if (filter === "closed") {
      return "Danh sách thuê bao đã đóng việc";
    }
  ...
  function openUpdate(row: Subscriber) {
    ...
    setForm({
      phone: row.phone || "",
      contact_status:
        row.latest_contact_status ||
        "CONTACTED",

      reason_l1_id:
        row.latest_reason_l1 || "",

      reason_l2_id:
        row.latest_reason_l2 || "",
  ...
  const isClosed =
    selected &&
    norm(selected.latest_workflow_status) ===
      "COMPLETED";

  const isNoAnswer =
    norm(form.contact_status) ===
    "NO_ANSWER";
  ...
  /*
   * Nếu thuê bao trước đó đã CONTACTED:
   * Backend hiện tại đang giữ nguyên nguyên nhân cũ.
   */
  const oldReasonLocked =
    selected &&
    norm(
      selected.latest_contact_status
    ) === "CONTACTED";

  /*
   * Khóa nguyên nhân khi:
   * - Không nghe máy
   * - TB đã đóng việc
   * - Hoặc thuê bao trước đó đã tiếp xúc
   */
  const reasonLocked =
    Boolean(isClosed) ||
    Boolean(isNoAnswer) ||
    Boolean(oldReasonLocked);
  ...
  async function saveRecovery() {
    ...
    if (
      norm(
        selected.latest_workflow_status
      ) === "COMPLETED"
    ) {
      setMessage(
    ...
    if (!normalizePhoneText(form.phone)) {
      setMessage(
        "Thuê bao chưa có số điện thoại. Vui lòng nhập bổ sung số điện thoại trước khi cập nhật."
      );
      return;
    }
     ...
     * =====================================================
     * ĐÃ TIẾP XÚC
     * => bắt buộc nguyên nhân cấp 1 + cấp 2
     * =====================================================
     */
    ...
    if (
      ...
      setMessage(
        "Vui lòng chọn nguyên nhân cấp 1 và cấp 2"
      );
      return;
    }
     ...
     * =====================================================
     * KHÔNG NGHE MÁY
     * => KHÔNG bắt nguyên nhân
     * => KHÔNG bắt kết quả xử lý
     * => cho phép lưu ngay
     * =====================================================
     */
    ...
    try {
      const payload = {
        ...
        phone: normalizePhoneText(
          form.phone
        ),

        /*
         * Không nghe máy:
         * đảm bảo không gửi nguyên nhân mới.
         */
        reason_l1_id:
          isNoAnswer
            ? ""
        ...
        /*
         * Không nghe máy và chưa chọn kết quả
         * thì gửi rỗng.
         */
        recovery_result:
          isNoAnswer
  ...
  function exportCsv() {
    ...
    a.download =
      `subscribers_${filter}_${filterVTKV}.csv`;
...
function MobileSubscriberCard({
  ...
  const reasonText =
    row.latest_reason_l2_name ||
    row.latest_reason_l1_name ||
    "Chưa cập nhật";

  const phone =
    normalizePhoneText(row.phone);
...
function MobileInfo({
  ...
  return (
    <div className="rounded-xl bg-slate-50 p-3">
      ...
      <div className="font-black text-slate-900 mt-1 break-all">
        {value}
      </div>
    </div>
  );
